...
<body>
    ...
    <div class="content">
        ...
        <h2>Dashboard</h2>
        <!-- chart container untuk looping kelas -->
        @foreach ($anggota as $anggota)
            <div class="chart-container">
                <h3>Progress Bar Hasil Belajar Siswa - {{ $anggota->kelas->nama_kelas }}</h3>

                <div class="chart-grid">
                    <!-- chart content untuk nampilin progress pribadi siswa-->
                    <div class="chart-content">
                        <span class="stud-name">{{ $anggota->siswa->nama }}</span>
                        <div class="chart-row">
                            <div class="prog-info">
                                <span class="skill-area">Penugasan</span>
                                <span class="percentage">
                                    @php
                                        $total = 0;
                                    @endphp
                                    @if (count($anggota->kelas->penugasan) != 0)
                                        @foreach ($anggota->kelas->penugasan as $penugasan)
                                            @if ($penugasan->prosesTugas->where('siswa_id', \Auth::guard('siswa')->user()->id)->first() != '')
                                                @php
                                                    if ($penugasan->prosesTugas->where('siswa_id', \Auth::guard('siswa')->user()->id)->first()->hasilTugas != '') {
                                                        
                                                        $total += $penugasan->prosesTugas->where('siswa_id', \Auth::guard('siswa')->user()->id)->first()->hasilTugas->nilai;
                                                    }
                                                @endphp
                                            @endif
                                        @endforeach
                                        {{ ceil((($total / count($anggota->kelas->penugasan)) * 100) / 100) }}%
                                    @endif
                                </span>
                            </div>
                            <div class="prog-bar">
                        ...
                        <div class="chart-row">
                            <div class="prog-info">
                                <span class="skill-area">Quiz Pilgan</span>
                                <span class="percentage">
                                    @php
                                        $total = 0;
                                    @endphp
                                    @if (count($anggota->kelas->quiz->where('tipe', 'Quiz Pilgan')) != 0)
                                        @foreach ($anggota->kelas->quiz->where('tipe', 'Quiz Pilgan') as $quiz)
                                            @foreach ($quiz->quizPilgan as $index => $qp)
                                                @if ($qp->prosesQuizPilgan->where('siswa_id', \Auth::guard('siswa')->user()->id)->first() != '')
                                                    @php
                                                        if ($index == 0) {
                                                            $total += $qp->prosesQuizPilgan->where('siswa_id', \Auth::guard('siswa')->user()->id)->first()->hasilQuizPilgan->nilai_total;
                                                        }
                                                    @endphp
                                                @endif
                                            @endforeach
                                        @endforeach
                                        {{ ceil((($total / count($anggota->kelas->quiz->where('tipe', 'Quiz Pilgan'))) * 100) / 100) }}%
                                    @endif
                                    </span>
                                </span>
                            </div>
                        ...
                        <div class="chart-row">
                            <div class="prog-info">
                                <span class="skill-area">Quiz Essay</span>
                                <span class="percentage">
                                    @php
                                        $total = 0;
                                    @endphp
                                    @if (count($anggota->kelas->quiz->where('tipe', 'Quiz Essay')) != 0)
                                        @foreach ($anggota->kelas->quiz->where('tipe', 'Quiz Essay') as $quiz)
                                            @foreach ($quiz->quizEssay as $index => $qe)
                                                @if ($qe->prosesQuizEssay->where('siswa_id', \Auth::guard('siswa')->user()->id)->first() != '')
                                                    @php
                                                        if ($index == 0) {
                                                            if ($qe->prosesQuizEssay->where('siswa_id', \Auth::guard('siswa')->user()->id)->first()->hasilQuizEssay != '') {
                                                                
                                                                $total += $qe->prosesQuizEssay->where('siswa_id', \Auth::guard('siswa')->user()->id)->first()->hasilQuizEssay->nilai_total;
                                                            }
                                                        }
                                                    @endphp
                                                @endif
                                            @endforeach
                                        @endforeach
                                        {{ ceil((($total / count($anggota->kelas->quiz->where('tipe', 'Quiz Essay'))) * 100) / 100) }}%
                                    @endif
                                </span>
                            </div>
                            <div class="prog-bar">
