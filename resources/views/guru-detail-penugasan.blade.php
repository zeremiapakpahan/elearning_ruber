        ...
        <div class="kelas-main-content">
            <div class="assignment-detail">
                <div class="comment-section">
                    ...
                    @if (Request::is('guru/kelas/penugasan*'))
                        ...
                        @foreach ($komen_siswa as $komen)
                            ...
                            <div class="comment-row" data-datetime="{{ $komen->created_at->format('d M Y H:i:s') }}" id="siswa-section">
                                <div class="comment-header">
                                    ...
                                    <span onclick="window.location.href='{{ url('guru/kelas/hapus-komentar-tugas-siswa/'.$komen->id) }}'"><i class="fa fa-trash"></i> Hapus </span>
                                </div>
                        ...
                        @foreach ($komen_guru as $komen)
                            <div class="comment-row" data-datetime="{{ $komen->created_at->format('d M Y H:i:s') }}" id="guru-section">
                                <div class="comment-header">
                                    ...
                                    <span onclick="window.location.href='{{ url('guru/kelas/edit-komentar-tugas/'.$komen->id) }}'"><i class="fa fa-pen"></i> Edit </span>
                                    <span onclick="window.location.href='{{ url('guru/kelas/hapus-komentar-tugas/'.$komen->id) }}'"><i class="fa fa-trash"></i> Hapus </span>
                                </div>
                                <div class="comment-content">
                                    {{ $komen->isi }}
                    ...
                    @if (Request::is('guru/kelas/edit-komentar-tugas*'))
                        {{-- <form class="comment-form" action=""> --}}
                        {!! Form::model($objek, ['action' => $action, 'method' => $method, 'files' => true, 'class' => 'comment-form']) !!}
                            {!! Form::hidden('tugas_id', $penugasan->id) !!}
                ...
                @if (Request::is('guru/kelas/penugasan*'))
                    <div class="submission-section">
                        <span class="submission-total">
                            Yang Mengumpulkan: <p>{{ count($penugasan->prosesTugas) }} Orang</p>
                        </span>
                        @foreach ($penugasan->prosesTugas as $hasil)
                            <div class="submission-row" data-datetime_sm="{{ $hasil->created_at->format('d M Y H:i:s') }}">
                                <div class="submission-header">
                                    @if ($hasil->siswa->foto != '')
                                        <img src="{{ \Storage::url($hasil->siswa->foto) }}" alt="Foto">
                                    @else
                                        <img src="{{ asset('project') }}/img/avatar.jpg" alt="Foto">
                                    @endif
                                    <span class="submission-identity">
                                        <span>{{ $hasil->siswa->nama }}</span>
                                        <span>{{ $hasil->siswa->status }}</span>
                                    </span>
                                </div>
                                <div class="submission-content">
                                    <div class="a-title">
                                        <span class="tag">Judul Tugas <p>:</p></span>
                                        <span>{{ $hasil->penugasan->judul }}</span>
                                    </div>
                                    <div class="submission-file">
                                        <span class="tag">Hasil Tugas <p>:</p> </span>
                                        <div>
                                            @foreach ($hasil->fileProsesTugas as $files)
                                ...
                                <span class="process-btn">
                                    <button onclick="window.location.href='{{ url('guru/kelas/hasil-tugas/'.$hasil->id) }}'" type="button" class="btn-section">Proses</button>
                                </span>
                                <span class="submission-datetime">
